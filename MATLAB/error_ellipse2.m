...
default_properties = struct(...
  ...
  'scale', [1,1], ... % Scale factor, e.g. 1e-3 to plot m as km
  'style', '', ...  % Plot style
  'clip', inf,... % Clipping radius
  'Fill', 0,... % Do or do not fill ellipse
  'FillColor', 'b',... % Ellipse fill color
  'EdgeColor', 'none',... % Ellipse edge color
  'FillAlpha', 1); % Ellipse fill transparency
...
elseif r==2 & c==2
  ...
  if prop.Fill==0
    h1=plot(scale(1)*(x0+k*x),scale(2)*(y0+k*y),prop.style);
  else
      h1 = patch(scale(1)*(x0+k*x),scale(2)*(y0+k*y),prop.FillColor,...
          'EdgeColor',prop.EdgeColor,'FaceAlpha',prop.FillAlpha);
  end
