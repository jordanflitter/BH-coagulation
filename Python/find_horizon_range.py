...
def nGC(z):
    return (cd.hubble_z(z,**cosmo)*4.34313792e+17)**3.  # hubble_z(z) in sec^-1

def sfr(z):
    return 0.015*(1.+z)**2.7/(1.+((1.+z)/2.9)**5.6)  #msun per yr per Mpc^3

#calculate the horizon distance/redshift
def find_horizon_range(m1,m2,asdfile,RzTdfile,approx=ls.IMRPhenomD):
	...
	#interpolate_psd = interp1d(input_freq, strain)				
	...
	if njump>=10:
		...
		try:
			interp_z=linspace(0.001,10.,200); interp_snr=zeros(size(interp_z))
			for i in range(0,size(interp_z)): 
				hplus_tilda,hcross_tilda,freqs= get_htildas((1.+interp_z[i])*m1,(1.+interp_z[i])*m2 ,cd.luminosity_distance(interp_z[i],**cosmo),fmin=fmin,fref=fref,df=df,approx=approx)
				fsel=logical_and(freqs>minimum_freq,freqs<maximum_freq)
				psd_interp = interpolate_psd(freqs[fsel]) 	
	...
	unit_comoving_volume=zeros(size(z));
	for i in range(0,size(z)):	
		...
		unit_comoving_volume[i]=cd.comoving_volume(z[i],**cosmo)/(1.+z[i])*P(w)
	...
	#average redshift
	vol_mean = sum(unit_volume*cd.comoving_volume(z,**cosmo))/vol_sum
	z_mean=sum(unit_volume*z)/vol_sum
	sfr_z_mean=sum(unit_volume*sfr(z)*z)/sfr_vol_sum
	sfr_vol_mean=sum(unit_volume*sfr(z)*cd.comoving_volume(z,**cosmo))/sfr_vol_sum
#	sfr_vol_mean=sum(unit_volume*sfr(z)*unit_comoving_volume)/sfr_vol_sum
	# correct rate for n_GC(z): multiply by (H(z)/H0)**3
	nGC_vol_sum=sum(unit_volume*nGC(z))
	nGC_vol_mean=sum(unit_volume*nGC(z)*cd.comoving_volume(z,**cosmo))/nGC_vol_sum
	# read redshift dependence of rate	
	#redshift_input,rates=loadtxt(RzTdfile,unpack=True,usecols=[0,1])
	#interpolate_RzTd = interp1d(redshift_input, rates)
	# correct rate for SFR and delay-time distribution
	#RzTd_vol_sum=sum(unit_volume*interpolate_RzTd(z))
	#RzTd_vol_mean=sum(unit_volume*interpolate_RzTd(z)*cd.comoving_volume(z,**cosmo))/RzTd_vol_sum

		
	return horizon_redshift,z_reach90,z_reach50


start = time.time()
import scipy.io as sio
Mmin = 1.0; Mmax = 1000.0; dM = 1.0
M_Vec= arange(Mmin,Mmax+dM,dM)
N=size(M_Vec)
M1,M2=meshgrid(M_Vec,M_Vec)
horizon_redshift=zeros((N)); z_reach90=zeros((N)); z_reach50=zeros((N));
for i in arange(N):
    print("Now calculating for i="+str(i+1)+" (out of N="+str(N)+")\n")
    horizon_redshift[i],z_reach90[i],z_reach50[i]=find_horizon_range(M1[i][i],M2[i][i],"aLIGO_O3a_Noise_Curve.dat","Rsfr_Delayed_by_50Myrs.dat")
    percents=100.*(i+1)/N
...
sio.savemat('Horizon_distances.mat',{'M_vec':M_Vec,'horizon_redshift':horizon_redshift,'z_reach90':z_reach90,'z_reach50':z_reach50})
'''
import cosmolopy.constants as cc
z = arange(0,1001,1)
t_z = cd.age(z, use_flat=True, **cosmo)/cc.Gyr_s*1e+9
interpolate_z = interp1d(t_z, z)
z = interpolate_z(arange(1,14,1)*1e+9)
